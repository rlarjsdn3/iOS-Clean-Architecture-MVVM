...
// Note: If autorization is needed NetworkSessionManager can be implemented by using,
...
final class DefaultNetworkErrorLogger: NetworkErrorLogger {
    ...
    func log(request: URLRequest) {
        ...
        if let httpBody = request.httpBody, let result = (try? JSONSerialization.jsonObject(with: httpBody) as? [String: String]) {
            printIfDebug("body: \(String(describing: result))")
        } else if let httpBoby = request.httpBody, let result = String(data: httpBoby, encoding: .utf8) {
            printIfDebug("body: \(String(describing: result))")
        }
    }
